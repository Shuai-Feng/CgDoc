          ...
          <List 
                    ...
                    <Card
                      actions={[
                        <Tooltip title="病床设置" arrowPointAtCenter>
                            <SettingOutlined key="setting"  onClick={()=>handleDrawerOpen('setting',item)}/>
                        </Tooltip>
                        ,
                        <Tooltip title="病例编辑" arrowPointAtCenter>
                            <EditOutlined key="edit"  onClick={()=>handleDrawerOpen('edit',item)}/>
                        </Tooltip>
                        ,
                        <Tooltip title="病患详情" arrowPointAtCenter>
                            <EllipsisOutlined key="ellipsis"  onClick={()=>handleDrawerOpen('ellipsis',item)}/>
                        </Tooltip>
                      ]}
                    >
                      <CardMeta
                        avatar={<Avatar style={item.patient_sex === 'male'?{ backgroundColor: '#0C6AFF' }:{ backgroundColor: 'hotpink' }}>{item.patient_name.substr(0,1)}</Avatar>}
                        title={<a>
                          {`${item.bed_num}号床 ${item.patient_name}
                          ${item.patient_sex === 'male'?'男':'女'} 
                          ${item.patient_age}岁`}</a>}
           ...
           >
              ...
              {dwTitle == 'edit'?
                ...
                form={diaryForm}
                  onFinish={(values)=>{
                     Axios.ajax({url:'/patient/diary',data:{
                       params:{
                         ...values,
                         patient_id:current_patient.patient_id
                       }
